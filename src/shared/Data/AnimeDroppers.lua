...
local function getStarterUnitBulkPrice(amount: number): number
	...
	local increment = math.max(math.floor(tonumber(TycoonConfig.UnitPriceIncrement) or 10), 0)

	if amount == 1 then
		return firstPrice
	end

	local linearPurchaseCount = amount - 1
	local lastPrice = secondPrice + (linearPurchaseCount - 1) * increment
	return firstPrice + linearPurchaseCount * (secondPrice + lastPrice) / 2
end

for tier, tierData in Tiers do
	local requiredTier1 = getRequiredTier1(tier)
	tierData.RequiredTier1 = requiredTier1
	tierData.EstimatedTier1Cost = getStarterUnitBulkPrice(requiredTier1)
end
